<template>
  ...
  <el-container class="container">
    ...
    <el-header class="header">
      <el-image src="https://cdn.staticaly.com/gh/Pitayafruits/myPicRep@main/PropertyManage/202301201508741.png"
        style="float: left"></el-image>
    ...
    <el-main>
      <el-row>
        ...
        <el-col :span="18">
          ...
          <!--物业公告、小区设施、物业人员-->
          <el-card class="left-card" style="margin-top: 20px">
            <el-tabs v-model="activeName" @tab-click="handleClick">
              ...
              <el-tab-pane label="物业公告" name="first">
                <el-table :data="noticeList" :show-header="false">
                  ...
                  <el-table-column prop="noticeTime" label="公告日期" :formatter="formatTime" width="180"></el-table-column>
                  <el-table-column label="操作">
                    <template slot-scope="scope">
                      <el-button size="small" @click="watchNotice(scope.row)" icon="el-icon-view">查看</el-button>
                    </template>
                  </el-table-column>
                </el-table>
                <el-pagination @size-change="noticeSizeChange" @current-change="noticeCurrentChange"
                  :current-page.sync="noticeParms.currentPage" :page-size="noticeParms.pageSize"
                  layout="total, prev, pager, next" :total="noticeParms.total"></el-pagination>
              </el-tab-pane>

              <!--小区设施-->
              <el-tab-pane label="小区设施" name="second">
                <el-tabs :tab-position="tabPosition" style="height: 200px">
                  <el-tab-pane :key="item.faName" v-for="item in facList" :label="item.faName">
                    <el-descriptions class="margin-top" title="设施信息" :size="size" :column="3" border>
              ...
              <el-tab-pane label="物业人员" name="third">
                <el-row>
                  <el-col :span="4" v-for="(item, index) in workerList" :key="index"
                    style="display: flex; flex-direction: column; align-items: center">
                    <el-card :body-style="{ padding: '0px', height: '207px' }">
                      <img
                        src="https://cdn.staticaly.com/gh/Pitayafruits/myPicRep@main/PropertyManage/202302211725193.jpg"
                        class="image" />
                    </el-card>
                    <div style="margin-top: 2px; font-size: 20px; font-weight: 500">{{ item.fullName }}</div>
                    <div style="margin-top: 4px">{{ item.phoneNumber }}</div>
                  </el-col>
                </el-row>
              </el-tab-pane>
            </el-tabs>
        ...
        <el-col :span="6">
          ...
          <el-card class="right-card">
            <div slot="header" class="clearfix">
              <span style="font-size: 24px; font-weight: 400; color: #409eff">用户登录</span>
            </div>
            <el-form :model="loginForm" ref="loginForm" :rules="rules" :inline="false" size="normal">
          ...
          <!--小区信息-->
          <el-card class="right-card" style="margin-top: 20px">
            <div slot="header" class="clearfix">
              <span style="font-size: 24px; font-weight: 400; color: #409eff">小区信息</span>
            </div>
            <el-table :data="tableList" style="width: 100%; margin-top: 15px" empty-text="暂无数据"
              :header-cell-style="{ background: 'rgba(0,0,0,0.05)' }">
              <el-table-column prop="commName" label="小区名称"></el-table-column>
            </el-table>
            <el-table :data="tableList" style="width: 100%" empty-text="暂无数据"
              :header-cell-style="{ background: 'rgba(0,0,0,0.05)' }">
              <el-table-column prop="roadArea" label="道路面积"></el-table-column>
              <el-table-column prop="greenArea" label="绿化面积"></el-table-column>
              <el-table-column prop="commArea" label="建筑面积"></el-table-column>
            </el-table>
            <el-table :data="tableList" style="width: 100%" empty-text="暂无数据"
              :header-cell-style="{ background: 'rgba(0,0,0,0.05)' }">
              <el-table-column prop="buildNum" label="楼宇数量"></el-table-column>
              <el-table-column prop="phoneNumber" icon="el-icon-phone-outline" label="联系我们"></el-table-column>
            </el-table>
          </el-card>
        </el-col>
      </el-row>
    ...
    <el-footer class="footer" height="40px">
      <div>
        <span class="footerFont">
          <img src="https://cdn.staticaly.com/gh/Pitayafruits/myPicRep@main/PropertyManage/202301201745065.png" />
          © 2023 物业管理系统 designed by wangyijie
        </span>
      </div>
    </el-footer>

    <!--弹框组件-->
    <my-dialog :title="dialog.title" :visible="dialog.visible" :height="dialog.height" :width="dialog.width"
      @onClose="onClose" @onConfirm="onConfirm">
      <template slot="content">
        <el-form :model="model" ref="addForm" :inline="false" size="normal">
          <el-form-item label="公告标题">{{ model.noticeTitle }}</el-form-item>
          <el-form-item label="公告内容">{{ model.noticeText }}</el-form-item>
        </el-form>
      </template>
    </my-dialog>

    <!--重置密码弹框-->
    <my-dialog :title="resetDialog.title" :visible="resetDialog.visible" :height="resetDialog.height"
      :width="resetDialog.width" @onClose="resetOnClose" @onConfirm="resetOnConfirm">
      <template slot="content">
        <el-form :model="form" ref="form" :rules="resetRules" :inline="true">
          <el-form-item prop="phone">
            <el-input prefix-icon="el-icon-phone" placeholder="请输入手机号" v-model="form.phone"
              style="width: 400px"></el-input>
          </el-form-item>
          <el-form-item prop="code">
            <el-row :gutter="20">
              <el-col :span="16">
                <el-input prefix-icon="el-icon-tickets" placeholder="请输入验证码" v-model="form.code"
